...
def run_single_dataset(dataset_info, seed):
    ...
    for algo_config in ALGORITHM_CONFIGS:
        ...
        try:
            ...
            mean_tree_depth = median_tree_depth = sd_tree_depth = np.nan
            mean_n_leaves = median_n_leaves = sd_n_leaves = np.nan
...
def main():
    ...
    output_dir = "experiments/experiment_raw_results/rf_empirical_study"
    ...
    overview_dir = (
        "experiments/experiment_analysis/rf_es_simulation_study/overview_csvs"
    )
    os.makedirs(overview_dir, exist_ok=True)
    overview_path = os.path.join(overview_dir, "rf_empirical_results.csv")
    df_agg_results.to_csv(overview_path, index=False)
    print(f"Aggregated results also saved to {overview_path}")
