OUTPUT_DIR = "experiments/experiment_analysis/dt_es_simulation_study/overview_csvs"
TEST_RUN = False

if TEST_RUN:
    MC_ITER = 1  # default value
    RANDOM_SEED = 7
    MAX_DEPTH = 30  # Maximum depth to try
    FILE_NAME = "depth_analysis_test.csv"
else:
    MC_ITER = 300
    RANDOM_SEED = 7
    MAX_DEPTH = 30  # Maximum depth to try
    FILE_NAME = "depth_analysis.csv"
...
def run_mc_iteration(mc_iteration, dgp_config):
    ...
    train_accuracies_per_depth = np.full(MAX_DEPTH + 1, np.nan)
    test_accuracies_per_depth = np.full(MAX_DEPTH + 1, np.nan)
    actual_depths_per_depth = np.full(
        MAX_DEPTH + 1, np.nan
    )  # New array for actual depths

    # Train separate trees for each depth
    for depth in range(0, MAX_DEPTH + 1):
        if depth == 0:
            no_split_clf = np.mean(y_train)
            train_accuracy = np.mean(y_train == (no_split_clf > 0.5))
            test_accuracy = np.mean(y_test == (no_split_clf > 0.5))
            actual_depth = 0
        else:
            dt = DecisionTreeClassifier(
                max_depth=depth, min_samples_split=2, max_features=None, random_state=7
            )
            dt.fit(X_train, y_train)
            train_accuracy = np.mean(dt.predict(X_train) == y_train)
            test_accuracy = np.mean(dt.predict(X_test) == y_test)
            actual_depth = dt.get_depth()  # Get actual depth reached

        train_accuracies_per_depth[depth] = train_accuracy
        test_accuracies_per_depth[depth] = test_accuracy
        actual_depths_per_depth[depth] = actual_depth
...
def main():
    ...
    for file in os.listdir(dgp_config_dir):
        if file.endswith(".yaml"):
            ...
            all_train_accuracies = np.zeros((MC_ITER, MAX_DEPTH + 1))
            all_test_accuracies = np.zeros((MC_ITER, MAX_DEPTH + 1))
            all_actual_depths = np.zeros((MC_ITER, MAX_DEPTH + 1))
            ...
            for depth in range(0, MAX_DEPTH + 1):
                if not np.isnan(mc_train_accuracies_per_depth[depth]):
                    results.update(
                        {
