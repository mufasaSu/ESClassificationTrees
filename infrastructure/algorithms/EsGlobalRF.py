...
from infrastructure.algorithms.clean_dt import (
    DecisionTreeLevelWise as ESDecisionTreeClassifier,
)
import infrastructure.noise_level_estimator as noise_est
import infrastructure.model_builder as model_builder
...
class RandomForestClassifier(object):
    ...
    def predict(self, X, n_trees=None):
        ...
        y_pred_labels_ensemble = np.round(
            ...
            int
        )  # Majority Voting: round rounds. <= 0.5 to zero, > 0.5 to one
        # return the prediction
        return y_pred_labels_ensemble

    def predict_proba(self, X, n_trees=None):  # TODO: Adjust to kappa and depth
        if n_trees is None:
            n_trees = self.n_estimators
        # check we've fit the ensemble
        if not self.estimators_:
    ...
    def get_mean_depth(self):
        ...
        """
        return np.round(
            np.mean([tree.get_depth() for tree in self.estimators_]), decimals=1
        )
    ...
    def get_sd_depth(self):
        ...
        """
        return np.round(
            np.std([tree.get_depth() for tree in self.estimators_]), decimals=2
        )
    ...
    def get_mean_n_leaves(self):
        ...
        """
        return np.round(
            np.mean([tree.get_n_leaves() for tree in self.estimators_]), decimals=1
        )
    ...
    def get_sd_n_leaves(self):
        ...
        """
        return np.round(
            np.std([tree.get_n_leaves() for tree in self.estimators_]), decimals=2
        )
